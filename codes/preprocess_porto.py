...
class DataFoursquare(object):
    def __init__(self, trace_min=10, global_visit=10, hour_gap=72, min_gap=10, session_min=2, session_max=10,
        ...
        tmp_path = "./DeepMove/data/"
        self.TWITTER_PATH = tmp_path + 'porto_checkin.txt'
        self.VENUES_PATH = tmp_path + 'foursquare/venues_all.txt'
        self.SAVE_PATH = tmp_path
        # self.save_name = 'foursquare_new'
        self.save_name = 'porto'
    ...
    def tid_list(tmd):
        # 使用time库将时间戳转换为本地时间
        local_time = time.localtime(int(tmd))
        # 格式化时间戳为可读的形式
        readable_time = time.strftime('%Y-%m-%d %H:%M:%S', local_time)
        tm = time.strptime(readable_time, "%Y-%m-%d %H:%M:%S")
    ...
    def save_variables(self):
        ...
        pk.dump(foursquare_dataset, open(self.SAVE_PATH + self.save_name + '.pk', 'wb'))
